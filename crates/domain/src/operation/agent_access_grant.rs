...
/// A HiRoute-local bearer generated from 256 bits of CSPRNG entropy.
///
...
impl AgentAccessGrantMaterial {
    ...
    pub fn from_authenticated_storage(bytes: Vec<u8>) -> Result<Self, OperationValidationError> {
        ...
        if !valid_material_encoding(&bytes) {
...
fn valid_material_encoding(bytes: &[u8]) -> bool {
    bytes.len() == MATERIAL_BASE64URL_BYTES
        && bytes.iter().all(|byte| BASE64URL.contains(byte))
        && BASE64URL
            .iter()
            .position(|candidate| candidate == &bytes[MATERIAL_BASE64URL_BYTES - 1])
