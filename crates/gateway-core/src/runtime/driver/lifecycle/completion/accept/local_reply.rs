...
{
    pub(super) async fn complete_accept_local_reply(
        &self,
        ...
        let local_reply =
            filter_local_reply.expect("local-reply completion is selected only for a staged reply");
        ...
        let mut terminal_result: Result<SessionReuse, GatewayExecutionError> = async {
            ...
            await_attempt_operation(
                ...
                exchange.finish_accepted_response(false),
            )
            .await?;
