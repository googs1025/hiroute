...
const outputRoot = path.resolve(process.argv[2] ?? path.join(desktopRoot, 'test-results/v3-session-window'));
...
const profile = await mkdtemp(path.join(os.tmpdir(), 'hiroute-v3-session-window-'));
