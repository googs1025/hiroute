...
type Preview = {
  applicable: boolean;
  blockers: { reason: string; capabilities?: { capability: string; reason: string }[] }[];
};
type Outcome = {
  preview: Preview;
...
function prerequisiteCheck(preview: Preview, facet: Facet, agent: Agent): CheckScope | null {
  ...
  if (agent.agent_id === 'agent_codex_default' && facet === 'collaboration' && (missing.has('skill_loading') || missing.has('trusted_cli_execution'))) return 'collaboration';
  return null;
}
    ...
    {preview.blockers.map((block, index) => {
      ...
      const message = capabilities.has('ingress_authentication')
        ...
        : block.reason === 'model_plan_unavailable' && selected?.agent_id === 'agent_codex_default' && defaultChoice.kind === 'preserve_native'
          ? text('当前 Codex 默认模型名称没有被所选智能路由或固定模型覆盖。请在“默认选择”中改选已勾选的路由；若要保留原生模型，先在“模型”中接入对应来源。配置未修改。', 'The current Codex default model name is not covered by a selected smart route or fixed model. Choose an enabled route under Default selection, or add the native model source under Models. Configuration was not changed.')
                ...
                {snapshot?.agents.map(agent => {
                  ...
                  const stateLabel = agent.status_error
                    ...
                    : executableState
                      ...
                      : statePending
                        ? text('正在更新', 'Updating')
                        : agent.settings?.current_selection || agent.settings?.collaboration?.current_selection
                          ? text('已接入智能路由', 'Routing connected')
                          : text('已发现 · 尚未接入', 'Detected · not connected');
                  return <button className={`list-row${active ? ' active' : ''}`} key={agent.agent_id} aria-current={active ? 'page' : undefined} onClick={async () => { if (dirty && !(await confirmDiscard(language))) return; setEditor(null); setPreview(null); setActionError(''); setNotice(''); setSelectedAgent(agent.agent_id); setShowAgentList(false); }}>
            ...
            <section className="detail-pane">
              ...
              {selected && snapshot && <div className="detail-inner" data-agent-id={selected.agent_id}>
                ...
                <section className="detail-section v3-agent-section">
                  ...
                  {modelSelection?.mode === 'claude_launcher' && selected.context_id && <p className="field-help"><code>{`hiroute agent launch --agent claude-code --context ${selected.context_id} -- <Claude arguments>`}</code></p>}
                </section>
                <section className="detail-section v3-agent-section">
                  ...
                  <form id="hr-agent-settings" className="agent-form" onSubmit={event => { event.preventDefault(); void change(selected, editor.facet, !facetEnabled, (event.nativeEvent as SubmitEvent).submitter as HTMLElement | null); }}>
                    <fieldset disabled={busy || !mutable}>
                      ...
                      {facetEnabled && editor.facet === 'model' && selected.agent_id === 'agent_codex_default' && <div className="worker-choices">
                        ...
                        {fixedModelRows.length > 0 && <fieldset data-agent-fixed-models><legend className="field-label">{text('已配置的原生模型固定来源', 'Configured native model sources')}</legend>
                          {fixedModelRows.map(model => {
                            ...
                            return <div className="candidate-row" key={model.client_model_id} data-client-model-id={model.client_model_id}>
                              <div><strong>{model.display_name}</strong><small><code>{model.client_model_id}</code></small><label className="field"><span className="field-label">{text('固定来源与账号范围', 'Fixed source and account scope')}</span><select className="select" value={fixed?.candidate.binding_id ?? ''} onChange={event => selectFixedSource(model, event.target.value)}><option value="">{text('不通过 HiRoute 固定此名称', 'Do not fix this name through HiRoute')}</option>{fixed && !source && <option value={fixed.candidate.binding_id} disabled>{text('原有来源当前不可用', 'Previous source unavailable')}</option>}{model.source_options.map(option => <option key={option.binding_id} value={option.binding_id} disabled={option.state !== 'ready'}>{option.source_label} · {option.account_scope_ref} · {sourceStateLabel(option.state)}</option>)}</select></label>{source && <span className="field-help">{sourceStateLabel(source.state)} · {text('账号范围摘要 ', 'Account scope digest ')}<code>{source.account_scope_digest.slice(0, 18)}…</code></span>}{fixed && source?.state === 'ready' && fixedReasoningControl(model, fixed, source)}{!model.source_options.length && <span className="field-help">{text('此原生名称尚无独立来源/账号绑定；元数据不会被当作可调用证明。', 'This native name has no independent source/account binding; metadata is not treated as callable proof.')}</span>}</div>
                            </div>;
                          })}
                        </fieldset>}
                        ...
                        <label className="field"><span className="field-label">{text('默认选择', 'Default selection')}</span><select className="select" data-agent-default value={defaultChoiceValue(editorValues.defaultChoice)} onChange={event => selectDefaultChoice(event.target.value)}><option value="native">{text('使用 Codex 当前默认模型名称', 'Use the current Codex default model name')}</option>{editorValues.fixedModels.map(model => <option key={model.client_model_id} value={`fixed:${model.client_model_id}`}>{model.client_model_id}</option>)}{editorValues.allowedPlanIds.filter(planEnabled).map(id => <option key={id} value={`plan:${id}`}>{planName(id)}</option>)}</select><span className="field-help">{defaultChoice.kind === 'preserve_native'
                          ...
                          : defaultChoice.kind === 'plan'
                            ? text('Codex 将默认使用所选智能路由；未选择的原生模型不会自动加入路由。', 'Codex will use the selected smart route by default; unselected native models are not added automatically.')
                      ...
                      {facetEnabled && editor.facet === 'model' && selected.agent_id === 'agent_claude_default' && <div className="worker-choices">
                        <p className="field-help">{text('受管启动器保留普通 Claude 参数、工作目录、TTY 和设置来源；这里只映射三个原生预设。可重复选择同一路由。', 'The managed launcher preserves ordinary Claude arguments, working directory, TTY, and setting sources. Only the three native presets are mapped here, and routes may be reused.')}</p>
