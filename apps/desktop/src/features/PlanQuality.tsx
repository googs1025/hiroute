  ...
  return <div className={`plan-quality${compact ? ' compact' : ''}`}>
    ...
    <div className="quality-list">{samples.map(sample => {
      ...
      return <article className="quality-row" key={sample.segment_id}>
        <div className="quality-row-main"><strong title={sample.model_configuration_id ?? undefined}>{modelName}</strong><span>{branchText(sample)} · turn {sample.first_turn_ordinal}–{sample.last_observed_turn_ordinal}</span><span>{new Date(sample.last_at_ms).toLocaleString(language === 'zh' ? 'zh-CN' : 'en')} · plan r{sample.plan_revision}</span></div>
        ...
        {sample.assessment && <p className="quality-coverage">{text(`评分覆盖 turn ${sample.assessment.target_from_ordinal}–${sample.assessment.target_through_ordinal}`, `Assessed turns ${sample.assessment.target_from_ordinal}–${sample.assessment.target_through_ordinal}`)}</p>}
